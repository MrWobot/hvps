...
std::shared_ptr<cJSON> TicketedSender::send(cJSON* request, uint64_t timeoutMilliseconds, CancellationToken* cancellationToken){	
	...
	TicketedSenderHandle* handle = new TicketedSenderHandle(
		static_cast<uint64_t>(TimeHelper::ms()) + timeoutMilliseconds);
	std::unique_lock<std::mutex> lock(_mutex);
	if(_disposed){
		delete handle;
		return nullptr;
	...
	return JHelper::toSharedPtr(responseMessage);
}
...
{
    const uint64_t millisecondsNow = static_cast<uint64_t>(TimeHelper::ms());

    std::unique_lock<std::mutex> lock(_mutexInstances);

    if (_instances.empty()) {
        _timerCheckTimedOutAllInstances.stop();  // ok to stop here
        return;
    }

    // Keep the lock while iterating to prevent use-after-free.
    for (auto* instance : _instances) {
        instance->checkTimedOuts(millisecondsNow); // grabs its own _mutex internally
    }
}
