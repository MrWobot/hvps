...
class Sampling{
	private:
		static inline constexpr int DEFAULT_WAIT_FOR_STATE_TIMEOUT_MS = 20000;
	public:
		...
		static bool runNCycles(HVPS_FPGAInterface& fpgaInterface, uint8_t nCycles,
			std::string& errorMessage, std::unique_ptr<uint8_t[]>& sampleBytes,
	...
	private:
		...
		static bool waitForState(HVPS_FPGAInterface& fpgaInterface, FPGAState state, int64_t timeoutMs  = DEFAULT_WAIT_FOR_STATE_TIMEOUT_MS);
		static void setCommand(HVPS_FPGAInterface& fpgaInterface, FPGACommand command);
		static void setInputData(HVPS_FPGAInterface& fpgaInterface, uint8_t nCycles);
	/*
		*/
};