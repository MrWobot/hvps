module HVPS_FPGAInterface (
    ...
    reg [23:0] input_staged;
    // Live input buffer
    reg [23:0] input_live;
    // Full output shift buffer (inputs + outputs)
    reg [1120:0] output_buffer;
    ...
    assign input_data = input_live[7:0];
    assign command = input_live[15:8];
    assign desired_output_voltage = input_live[23:16];
    assign out_value = output_buffer[1120];
    ...
    always @(posedge clk) begin
        ...
        if (in_shift_debounced && !in_shift_prev) begin
            input_staged <= {input_staged[22:0], in_value};
        end
        ...
        if (to_output_debounced && !to_output_prev) begin
            output_buffer <= {done_command, actual_first_stage_voltage, actual_output_voltage, actual_peak_primary_current, state, max_first_stage_voltage, max_output_voltage, max_primary_current, echo_desired_output_voltage, error, buffered_data, input_staged};
        end else if (out_shift_debounced && !out_shift_prev) begin
            output_buffer <= {output_buffer[1119:0], 1'b0};
        end
